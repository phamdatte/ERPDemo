...
import { Vendor } from '@/api/procurement.api';
...
export function PurchaseOrderModal({ vendors, products, mutation, onClose }: Props) {
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-modal w-full max-w-2xl flex flex-col max-h-[90vh]">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
          <h2 className="text-lg font-semibold text-slate-800">Lập đơn đặt mua</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4 space-y-4 overflow-y-auto">
          <div className="grid grid-cols-2 gap-4">
            <Input label="Số đơn" name="orderNumber" value={form.orderNumber} onChange={(e) => setForm({ ...form, orderNumber: e.target.value })} />
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Ngày đặt</label>
          ...
          <div>
            ...
            <select required value={form.vendorId} onChange={(e) => setForm({ ...form, vendorId: e.target.value })} className="input-base">
              ...
              {lines.map((line, i) => (
                <div key={i} className="grid grid-cols-12 gap-2 items-end">
                  ...
                  <div className="col-span-3">
                    <input type="number" min="0" placeholder="Đơn giá" value={line.unitPrice} onChange={(e) => updateLine(i, 'unitPrice', e.target.value)} className="input-base text-sm" />
                  </div>
