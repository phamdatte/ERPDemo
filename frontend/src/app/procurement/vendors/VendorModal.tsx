...
export function VendorModal({ vendor, onClose }: Props) {
  ...
  const [form, setForm] = useState({ code: '', name: '', contactName: '', phone: '', email: '', address: '' });
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-modal w-full max-w-lg flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
          <h2 className="text-lg font-semibold text-slate-800">{isEdit ? 'Sửa nhà cung cấp' : 'Thêm nhà cung cấp'}</h2>
